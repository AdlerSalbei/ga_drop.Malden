...
if (hasInterface) then {
    _endMission = {
        ...
        [_winningSide,_endText] spawn {
            ...
            if (missionNamespace getVariable ["drop_endInProgressClient", false]) exitWith {INFO("A different ending is already in progress.")};
            drop_endInProgressClient = true;
    ...
    //dedicated
    if (!isServer) then {
        "drop_gameEnded" addPublicVariableEventHandler _endMission;
    };

    //localhost
    if (isServer) then {
        [{count (missionNamespace getVariable ["drop_gameEnded", []]) > 0}, {
            ["drop_gameEnded", drop_gameEnded] call (_this select 0);
